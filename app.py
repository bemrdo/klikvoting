...
def get_report(id):
    ...
    cur = mysql.connection.cursor()
    resultValue = cur.execute("SELECT h.id_hash, h.encrypted, h.created_at, h.role, h.status, v.name, c.name FROM h_{} h LEFT JOIN v_{} v on h.id_hash = v.status LEFT JOIN c_{} c on h.id_hash = c.status".format(id, id, id))
    if resultValue > 0:
