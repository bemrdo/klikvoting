...
def get_report(id):
    ...
    cur = mysql.connection.cursor()
    resultValue = cur.execute("SELECT h.id_hash, h.id_candidate, h.encrypted, h.created_at, h.role, h.status FROM h_{} h LEFT JOIN v_{} v on h.id_hash = v.status".format(id, id))
    if resultValue > 0:
