...
def report_voting(id_voting):
    ...
    options = {
        ...
        'footer-left':(datetime.now() + timedelta(hours = 8)).strftime('%d/%m/%Y %I:%M %p'),
        'footer-font-size':10,
        'no-outline':None
    }
...
def organizer_dashboard():
    ...
    if request.method == 'POST':
        ...
        if data['submit'] == 'create-voting':
            ...
            created_at = str(datetime.now() + timedelta(hours = 8))
            cur = mysql.connection.cursor()
            cur.execute("INSERT INTO voting(id_voting, id_user, name, voting_desc, date_start, date_end, candidate, voter, viewer, created_at, last_checked) "\
            "VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (id_voting, id_user, name, voting_desc, date_start, date_end, candidate, voter, viewer, created_at, created_at))
            mysql.connection.commit()
...
def get_report_finish(id):
    cur = mysql.connection.cursor()
    resultValue = cur.execute("SELECT c.name, c.id_candidate, COUNT(h.id_candidate) as res FROM c_{} c LEFT JOIN h_{} h ON c.id_candidate = h.id_candidate GROUP BY c.id_candidate ORDER BY res DESC, c.created_at".format(id, id))
    if resultValue > 0:
        report = []
        candidates = cur.fetchall()
        for candidate in candidates:
...
def get_count(id):
    cur = mysql.connection.cursor()
    resultValue = cur.execute("SELECT c.id_candidate, c.name, c.description, c.avatar, COUNT(h.id_candidate) AS result FROM c_{} c LEFT JOIN h_{} h ON c.id_candidate = h.id_candidate GROUP BY c.id_candidate ORDER BY result DESC, c.created_at".format(id, id))
    if resultValue > 0:
